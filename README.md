...
- 波形は「プレビュー」「分割点を編集」の2モード。プレビューはタップした位置から再生(速度1x/1.25x/1.5x/2x切替可)。分割点を編集モードでは、演奏区間内をタップして分割点を追加/削除できます(オレンジの縦線)。再判定を行うと手動分割点はリセットされます
...
- **ズーム&ページング**: 既存ツールにあった5分/2分/1分単位のズーム表示は、今回は移植していません。全体波形のみの表示です。
