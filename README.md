...
- 検出された各曲には個別のトリム編集(前後の開始/終了位置をドラッグで微調整、波形をタップした位置から範囲内プレビュー)と、音量調整(-18dB〜+18dB)がついています。隣の曲の範囲までは動かせないようになっています。音量調整はプレビュー再生にも、保存時のWAV書き出しにも反映されます(共有時のMP3にもそのまま引き継がれます)。**波形自体も音量調整に合わせて表示が変わり、音が割れる(クリップする)部分は波形が警告色になります**

## 今後の計画メモ

- **簡易コンプレッサー**: Web AudioのDynamicsCompressorNodeを使えば、パラメータを細かく出さずに「軽くかける/かけない」程度のシンプルな適用は可能
